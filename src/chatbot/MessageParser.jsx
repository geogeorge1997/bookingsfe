...
import { setLoadCount, setNameMessageFlag, setAgeMessageFlag, setName } from '../redux/index'

const MessageParser = ({ children, actions }) => {
  const dispatch = useDispatch();

  const loadCount = useSelector((state) => state.chatbotReducer.loadCount)
  ...
  useEffect(() => {
    if (loadCount===0) {
      dispatch(setLoadCount(loadCount + 1))
    }

    if (loadCount === 1 && initialMessageFlag) {
      setTimeout(() => {
  ...
  }, [loadCount, initialMessageFlag, welcomeMessageFlag, calendarMessageFlag, nameMessageFlag,ageMessageFlag]);
